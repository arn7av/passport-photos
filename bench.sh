...
# Check imports and decode the image before replacing any previous benchmark output.
run_uv python -c '
import sys
import cv2, deepface, onnxruntime, rembg, segment_anything, torch, transformers
...
run_pp() {
  run_uv python "$SCRIPT_DIR/process_image.py" "$@"
}
...
echo "=== Face detection backends separately (original background, untiled) ==="
for backend in haar yunet deepface retinaface mtcnn auto; do
  echo "--- face backend: $backend (pipeline) ---"
  ...
  echo "--- face backend: $backend (list-faces only) ---"
  run_pp "$INPUT" --list-faces --face-backend "$backend" --yunet-model "$YUNET_MODEL" -v 2>&1 | tee "$OUT_DIR/face_${backend}_list.log" | summarize
done

echo ""
echo "=== Background backends separately (fixed face=haar, untiled) ==="
echo "--- background backend: none with face Haar ---"
...
for model in bria-rmbg birefnet-general birefnet-portrait; do
  echo "--- background backend: rembg model=$model with face Haar ---"
...
echo "--- background backend: BiRefNet_dynamic with face Haar ---"
...
echo "--- background backend: BEN2 with face Haar ---"
...
echo "--- background backend: sam vit_l with face Haar ---"
...
echo "--- background backend: sam vit_h with face Haar ---"
...
echo "--- background backend: sam vit_b with face Haar ---"
...
echo "--- background backend: auto with face Haar (fixed quality-ranked chain) ---"
...
echo ""
echo "Bench done. Outputs and full logs: $OUT_DIR/"
